...
void P_PlayerInSpecialSector (player_t* player)
{
    ...
    break;
	  ...
	  case 15:
	// VROOM: MUD SECTOR (SLOW)
    player->mo->momx = FixedMul(player->mo->momx, FRACUNIT / 2);
    player->mo->momy = FixedMul(player->mo->momy, FRACUNIT / 2);
	break;
    ...
    break;
	  ...
	  case 18:
	// VROOM: TRAMPOLINE (LOW)
    S_StartSound(player->mo, sfx_pstart);
    player->mo->momz = 10 * FRACUNIT;
    break;

	  case 19:
	// VROOM: TRAMPOLINE (HIGH)
    S_StartSound(player->mo, sfx_pstart);
    player->mo->momz = 20 * FRACUNIT;
    break;

	  case 20:
    // VROOM: HEALING SECTOR (10 HP)
    if (player->health < 100 && !(leveltime & 0x1f))
    {
        player->health += 10;
    ...
    // VROOM: ICE SECTOR
    {
        player->mo->momx = FixedMul(player->mo->momx, 110 * FRACUNIT / 100);
        player->mo->momy = FixedMul(player->mo->momy, 110 * FRACUNIT / 100);

        {
            fixed_t max_speed = 32 * FRACUNIT;
            fixed_t speed = P_AproxDistance(player->mo->momx, player->mo->momy);
            if (speed > max_speed)
            {
                player->mo->momx = FixedMul(player->mo->momx, FixedDiv(max_speed, speed));
                player->mo->momy = FixedMul(player->mo->momy, FixedDiv(max_speed, speed));
            }
        }
    }
    break;
